...
try {
    while ($ws.State -eq [System.Net.WebSockets.WebSocketState]::Open) {
        ...
        # if ($data.state.ButtonEvent -or $data.state.Presence) {
            ...
            foreach ($TriggerGroup in $CurrentSensor.TriggerGroup) {
                ...
                if ($data.state.ButtonEvent) {
                    ...
                    Write-Host "Sensor: $($CurrentSensor.Name):$($CurrentSensor.ApiId), Group: $($Group.Name):$($Group.id), Presence detected: $PresenceDetected, Ignore Daylight: $IgnoreDaylightSetting, Real dark: $isDark, Effective dark (Power state): $EffectiveDark"
                }
            }
        }
    }
